...
> 最近更新：2026-07-24（**完成度验收 G4 交付**——学习者模型 7 项实测 ✅ 无修复批：G4 脚本 e2e 验证 concepts 注册+材料挂接/evidence 三路/衰减+封顶/迁移预览应用；4.4-4.6 走查 9f 打勾）
...
**当前阶段 = 完成度验收**：G1 ✅ / G2 ✅ / G3 ✅（含修复批）/ G4 ✅（无修复批）；G5-G12 待办。按 `docs/AcceptanceChecklist.md` 逐项检查（实测打勾，发现问题开修复批：分支 + 三件套全绿 + 双子审查 + 合并 push）。mark_wrong 工具（§9）与「command 失败外部落盘不回滚」修复批仍留档另立。
