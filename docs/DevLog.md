...
> 最近更新：2026-07-22（P1-3 间隔复习 + P1-2 增量式 Study.md 完成，56 单测/49 走查全绿）
...
- 测试：`python -m unittest discover -s tests` → 56 个全绿；UI 走查 49 项全绿
- ⚠️ 走查结束会 `POST /api/session/reset` 清测试消息——**有值得保留的对话时不要跑走查**

## 下一步（已规划，见 docs/Roadmap.md）

~~P0-1 tool-use~~、~~P0-2 Mermaid~~、~~P1-2 增量 Study.md~~、~~P1-3 间隔复习~~ 已完成（2026-07-22）。
