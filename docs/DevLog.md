...
> 最近更新：2026-07-24（**完成度验收 G8 交付**——双模式 6 项实测 ✅ 无修复批：G8 脚本验证 mode 落盘+刷新布局恢复；8.3/8.6 单测、8.1/8.4 走查打勾）
...
**当前阶段 = 完成度验收**：G1-G8 ✅（G2/G3/G6 含修复批）；G9-G12 待办。按 `docs/AcceptanceChecklist.md` 逐项检查（实测打勾，发现问题开修复批：分支 + 三件套全绿 + 双子审查 + 合并 push）。mark_wrong 工具（§9）与「command 失败外部落盘不回滚」修复批仍留档另立。
