...
python -m unittest discover -s tests                                  # 全部测试（379 个，stdlib unittest）
python resources/hooks/validate_study.py <docx_dir> [total_days] [replica_name]  # 改学习数据后必跑
python scripts/ui_walkthrough.py                                      # UI 走查 137 项（需服务运行中；会清测试工作区聊天历史）
```
