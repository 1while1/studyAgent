...
- **演进方向（封板）**：`docs/AgentDesign.md` v3 —— 学习者模型核心的 study/code 双模式 Agent，M1-M7 分期。**M1 资料库 / M2 可观测已交付**，下一步 = M3 学习者模型
...
python -m unittest discover -s tests                                  # 全部测试（147 个，stdlib unittest）
python resources/hooks/validate_study.py <docx_dir> [total_days] [replica_name]  # 改学习数据后必跑
python scripts/ui_walkthrough.py                                      # UI 走查 58 项（需服务运行中；会清测试工作区聊天历史）
```
