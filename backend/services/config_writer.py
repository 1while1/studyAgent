...
并发：全部读→改→写函数共用一把进程内 RLock（M6 审查修复 E1）——
update_code_roots/update_toml_sections/update_workspaces/update_env_file
互斥执行，防并发覆盖丢更新（如 scaffold 注册代码根与模型配置页保存并发）。
"""
...
def update_code_roots(path: Path, roots: list[dict]) -> None:
    ...
    ⚠️ roots 必须是**全量未过滤**清单（含其他工作区的根）——调用方若传入
    按当前工作区过滤后的 config.code_roots，会静默丢弃别的工作区的根
    （M6 审查修复 R1 的真实数据丢失事故）。
    """
    with _SETTINGS_LOCK:
        text = path.read_text(encoding="utf-8")
        lines = text.splitlines()
